(function () {
  ...
  function initLandingSlides() {
    ...
    const slides = [...track.querySelectorAll('.landing-slide')]
    const dots = [...document.querySelectorAll('[data-landing-progress] button')]
    let index = 0
    let locked = false
  ...
  function initReveals() {
    const items = [...document.querySelectorAll('.section__inner, .card, .profile-panel, .profession-card, .result-card')]
  ...
  function initAscii() {
    ...
    document.querySelectorAll('[data-ascii-logo]').forEach((node) => {
      ...
      const chars = [...base]
      const visible = chars.map((char, index) => (char.trim() ? index : -1)).filter((index) => index >= 0)
      window.setInterval(() => {
        const next = [...chars]
  ...
  function initTyped404() {
    ...
    if (!typed) return
    const messages = JSON.parse(typed.getAttribute('data-messages') || '[]')
    let messageIndex = 0
    let charIndex = 0
    const tick = () => {
